...
RSpec.describe "bundle outdated" do
  ...
  describe "with multiple, duplicated sources, with lockfile in old format", :bundler => "< 3" do
    before do
      ...
      gemfile <<~G
        source "#{endpoint_uri_for(gem_repo2)}"

        gem 'dotenv'

        source "#{endpoint_uri_for(gem_repo3)}" do
          gem 'pkg-gem-flowbyte-with-dep'
        end

        gem 'vcr', source: "#{endpoint_uri_for(gem_repo2)}"
      G

      lockfile <<~L
        GEM
          remote: #{endpoint_uri_for(gem_repo2)}/
          remote: #{endpoint_uri_for(gem_repo3)}/
          specs:
            dotenv (2.7.6)
            oj (3.11.3)
